...
).allowExcessArguments(false).option("-v, --verbose", "Show full listing of changes").option(
  "--dry-run",
...
).addOption(new Option("--debug-bundle-path <path>").hideHelp()).addOption(new Option("--debug").hideHelp()).addOption(new Option("--admin-key <adminKey>").hideHelp()).addOption(new Option("--url <url>").hideHelp()).addOption(new Option("--write-push-request <writePushRequest>").hideHelp()).addOption(
  new Option(
    "--preview-name <name>",
...
).addOption(new Option("--live-component-sources").hideHelp()).showHelpAfterError().action(async (cmdOptions) => {
