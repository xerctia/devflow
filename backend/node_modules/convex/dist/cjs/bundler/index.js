...
async function bundle(ctx, dir, entryPoints2, generateSourceMaps, platform, chunksFolder = "_deps", externalPackagesAllowList = [], extraConditions = []) {
  ...
  for (const warning of result.warnings) {
    (0, import_context.logWarning)(ctx, import_chalk.default.yellow(`esbuild warning: ${warning.text}`));
  }
...
async function entryPoints(ctx, dir) {
  ...
  for (const { isDir, path: fpath, depth } of walkDir(ctx.fs, dir)) {
    ...
    if (relPath.startsWith("_generated" + import_path.default.sep)) {
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping ${fpath}`));
    } else if (base.startsWith(".")) {
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping dotfile ${fpath}`));
    } else if (base.startsWith("#")) {
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping likely emacs tempfile ${fpath}`));
      ...
      (0, import_context.logVerbose)(
        ctx,
        import_chalk.default.yellow(`Skipping ${fpath} that contains multiple dots`)
      );
      ...
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping ${fpath} declaration file`));
    } else if (base.endsWith(".tsbuildinfo")) {
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping ${fpath} tsbuildinfo file`));
    } else if (base.endsWith(".json")) {
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping ${fpath} json file`));
    } else if (base.endsWith(".jsonl")) {
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping ${fpath} jsonl file`));
    } else {
      (0, import_context.logVerbose)(ctx, import_chalk.default.green(`Preparing ${fpath}`));
